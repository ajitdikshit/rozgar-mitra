...
export default function History() {
  ...
  useEffect(() => { api.get("/worker/history").then(r => setList(r.data)); }, []);
  return (
    <div className="app-shell pb-24">
      <Navbar title={t.history} onBack={() => nav(-1)}/>
      <div className="p-4 space-y-3">
        {list.length === 0 && <p className="text-center text-[#4A5568] py-12" data-testid="no-history">No verified jobs yet.</p>}
        {list.map(r => (
